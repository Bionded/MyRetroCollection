...
parser.add_argument('--sort', help='Sorts all your roms and stores them by system in a new directroy structure',
                    nargs=1)
...
parser.add_argument('--listmissing', help='creates a list of missing games per system on the file of your choice',
                    nargs=1)
...
parser.add_argument('--nonamemodif', help='Do not add game name modifiers as found in filename (Version/disk/etc)',
                    action='store_true')
parser.add_argument('--region',
                    help='select region for scraping, defualt is world (wor), some examples us,jp,eu,fr,sp ', nargs=1)
parser.add_argument('--language',
                    help='select language for scraping, defualt is english (en), some examples es,de,fr,es ', nargs=1)
        ...
        if self is not None:
            ...
            for attr in attrs:
                if isinstance(attrs[attr], dict):
                    ...
                    logging.debug(
                        '####### ADDING NODE ' + str(type(attr)) + ' WITH VALUE ' + str(type(attrs[attr]['text'])))
...
def getGameName(jsondata, path):
    ...
    if 'noms' in jsondata['jeu']:
        logging.debug('###### NAMES IS NOT A DICT SO CONVERTING ' + str(jsondata['jeu']['noms']) + ' TO ' + str(
            jsondata['jeu']['noms'][0]))
...
def callAPIURL(URL, forceCall=False):
    ...
    while not successCall:
        ...
        if response == 'QUOTA':
            logging.debug(
                '###### WE GOT QUOTA ERROR FOR ANON AND IDENTIFIED - WE CANNOT CONTINUE - WAIT 10 MINS TO RETRY')
            sleep(180)
...
def waitNewDay(runTime):
    ...
    params['gameid'] = 1
    while (
            startTime > datetime.today().time()) and not allowed:  # you can add here any additional variable to break loop if necessary
        sleep(60)
...
def callAPI(URL, API, PARAMS, CURRSSID, Version='', tolog='', returnRaw=False, forceCall=False):
    ...
    if '{' in response:
        ...
        try:
            result = response.replace('\x0d\x0a', '\\r\\n').replace('\x0a', '').replace('\x09', '').replace('\x0b',
                                                                                                            '').replace(
                '  ', '').replace('\r', '')
            ...
            try:
                new_response = result.replace('\x0d\x0a', '\\r\\n').replace('\x0a', '').replace('\x09', '').replace(
                    '\x0b', '').replace('  ', '').replace('\r', '')
            except Exception as e:
                logging.error('###### COULD NOT REPLACE JSROMINFO ' + str(e))
    ...
    if isinstance(retJson, (dict, list)):
        try:
            ...
            logging.debug(
                '###### V2 QUOTA INFO - OK CALLS ' + str(okcalls) + ' KO CALLS ' + str(kocalls) + ' OK MAX ' + str(
                    okmax) + ' KO MAX ' + str(komax))
            ...
            if 'ssuser' in str(e):
                logging.debug(
                    '###### ERROR IN GETTING INFORMATION FROM RESPONSE // PROBABLY ANON CALL [SSUSER MISSING] , NOTHING TO WORRY ABOUT ' + str(
                        e))
...
def grabVideo(URL, destfile):
    ...
    while (result != 0) and (retries < 10):
        ...
        logging.debug(
            '###### COMMAND: wget --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 --tries=5 -c -q ' + str(
                URL) + ' -O ' + str(destfile))
        try:
            result = subprocess.call(
                ['wget', '--retry-connrefused', '--waitretry=1', '--read-timeout=20', '--timeout=15', '--tries=5', '-c',
                 '-q', URL, '-O', destfile])
        except Exception as e:
    ...
    if (not (os.path.isfile(destfile)) and ('images' in destfile)) or UPDATEDATA:
        ...
        if img1 != '':
            ...
            try:
                ...
                logging.error(
                    '###### THIS REALLY SHOULD NOT HAPPEN - VERIFY YOUR FILE SYSTEM, SPECIFICALLY SYMLINK PERMITS')
            return
...
def processFile(path, file, CURRSSID, doRename, sysid):
    ...
    logging.debug('##### GETTING GAME INFO ' + str(path) + '/' + str(file) + ' ' + str(md5offile) + ' ' + str(
        sha1offile) + ' ' + str(crcoffile))
        ...
        for file in filelist:
            ...
            if gameinfo is not None:
                ...
                try:
                    ...
                    logging.debug(
                        '###### STRANGE, THIS ID WAS NOT IN THE LIST OF GAMES FOR THE SYSTEM, POSSIBLY MULTIDISC ' + str(
                            e))
...
def locateShainDB(mysha1='None', mymd5='None', mycrc='None', filename='', sysid=0, path=''):
    ...
    if result is not None:
        ...
        if gameID is not None:
            try:
                myRom = {'romfilename': filename, 'romsha1': mysha1.upper(), 'romcrc': mycrc.upper(),
                         'rommd5': mymd5.upper(), 'beta': '0', 'demo': '0', 'proto': '0', 'trad': '0', 'hack': '0',
                         'unl': '0', 'alt': '0', 'best': '0', 'netplay': '0'}
                myRoms = [myRom]
            except Exception as e:
...
def getAllGames(sysid):
    ...
    try:
        ...
        if isinstance(gameList[0], int):
            logging.error(
                '###### I COULD NOT IDENTIFY THE SYSTEM, CHECK YOUR es_systems.cfg FILE SO THE SSNAME TAG FOR THIS SYSTEM MATCHES THE SCREENSCRAPER CONFIG')
            ...
            for rom in result[0]:
                ...
                for drom in collection:
                    ...
                    if rom[0][:rom[0].rindex('.') + 1].lower() in drom[1].lower():
                        ...
                        while retries > 0:
                            try:
                                command = ['wget', '--retry-connrefused', '--waitretry=1', '--read-timeout=20',
                                           '--timeout=15', '--tries=5', '-c', '-q', romurl, '-O', destfile]
                        ...
                        if retries > 0:
                            ...
                            logging.error(
                                '####### FAILED TO wget --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 --tries=5 -c -q ' + romurl + ' -O ' + destfile)
    return missingids
                    ...
                    if system.tag == 'ssname':
                        ...
                        for mysystem in systems:
                            ...
                            for apisysname in mysystem['noms']:
                                logging.debug('###### COMPARING ' + sysname.upper() + ' WITH ' + mysystem['noms'][
                                    apisysname].upper())
...
def getArcadeName(name):
    ...
    try:
        ...
        gamename = re.findall("<title>(.*?)<\/title>", response)[0].replace('Game Details:  ', '').replace(
            ' - mamedb.com', '')
        ...
        try:
            ...
            try:
                ...
                gamename = re.findall("<title>(.*?)<\/title>", response)[0].replace(' - MAME machine', '').replace(
                    ' - MAME software', '')
...
def insertGameDataInLocalDB(mygame, doupdate):
    ...
    if result != None:
        ...
        values = (mygame['notgame'], mygame['topstaff'], mygame['rotation'], mygame['cloneof'], mygame['system'],
                  mygame['editeur'], mygame['joueurs'], mygame['id'])
...
def insertSystemInLocalDb(system):
    ...
    if (not result) or result == []:
        ...
        values = (
        system['id'], system['noms']['nom_eu'], system['type'], int(parent), recalbox, retropie, launchbox, hyperspin)
        ...
        values = (
        system['noms']['nom_eu'], system['type'], int(parent), recalbox, retropie, launchbox, hyperspin, system['id'])
...
def insertGameRomsInDB(id, roms, sysid, doupdate):
    ...
    for rom in roms:
        ...
        if result is None or result == []:
            ...
            values = (
            rom['romfilename'], romsha1, romcrc, rommd5, str(rom['beta']), str(rom['demo']), str(rom['proto']), \
...
def insertGameMediasInDB(id, medias, sysid, doupdate):
    ...
    for media in medias:
        try:
            ...
            mediaregion = 'UNK'
        try:
            sql = 'SELECT id FROM gameMedias where gameid=' + str(
                id) + ' and region ="' + mediaregion + '" and type = "' + media['type'] + '" and format="' + media[
                      'format'] + '"'
        except Exception as e:
        ...
        if result == None or result == []:
            ...
            if mediaregion == 'UNK':
                staticURL = 'www.screenscraper.fr/medias/' + str(sysid) + '/' + str(id) + '/' + media['type'] + '.' + \
                            media['format']
            else:
                staticURL = 'www.screenscraper.fr/medias/' + str(sysid) + '/' + str(id) + '/' + media[
                    'type'] + '(' + mediaregion + ').' + media['format']
